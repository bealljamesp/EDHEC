...
def bond_cash_flows(maturity, principal, coupon_rate, coupons_per_year):
    """
    Returns the series of cash flows of a bond, indexed by the period number.
    """
    n_periods = int(maturity * coupons_per_year)
    coupon_amt = (principal * coupon_rate) / coupons_per_year

    # Create an array of cash flows where every period gets the coupon,
    # and the final period also gets the principal repayment.
    # Using modern Python / NumPy types:
    cf = np.repeat(coupon_amt, n_periods)
    cf[-1] += principal
